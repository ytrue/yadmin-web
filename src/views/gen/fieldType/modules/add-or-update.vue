        ...
        <a-spin :spinning="confirmLoading">
            <a-form :form="form">
                ...
                <a-form-item label="字段类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-input placeholder="字段类型"
                             v-decorator="['columnType',{rules: [{required: true, message: '请输入字段类型'}]}]"/>
                </a-form-item>

                <a-form-item label="属性类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-input placeholder="属性类型"
                             v-decorator="['attrType',{rules: [{required: true, message: '请输入属性类型'}]}]"/>
                </a-form-item>

                <a-form-item label="属性包名" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-input placeholder="属性包名"
        ...
        methods: {
            ...
            init(id) {
                ...
                Api.detail(this.formId).then((response) => {
                    ...
                    this.confirmLoading = false
                    //设置值
                    const data1 = _.pick(response.data.data, ['columnType', 'attrType', 'packageName']);
                    // 渲染之前不能加入填充数据...
                    this.$nextTick(() => {
